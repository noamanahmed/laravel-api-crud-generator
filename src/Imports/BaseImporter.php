...
abstract class BaseImporter implements BaseImporterContract
{
    ...
    {
        if (!$this->importRelationsShips) return;
        $relations = [];
        foreach ($this->relations() as $relation) {
            // Only run if set to run BEFORE main importer
            if (! ($relation['runBeforeMainImporter'])) continue;
            $relations[] = $relation;
        }
    ...
    public function importRelationsShip($data, $relation, $columns)
    {
        ...
        if (isset($relation['relationType']) && $relation['relationType'] === 'parent') {
            ...
            foreach ($relationData as $key => $column) {
                if (in_array($key, $objModel->getFillable())) continue;
            ...
            foreach ($dataChunks as $chunk) {
                $chunk = $chunk->map(function ($row) use (&$objModel, &$importer) {
                    foreach ($row as $key => $column) {
                        if (in_array($key, $objModel->getFillable())) continue;
                        if (in_array($key, $importer->uniqueColumns())) continue;
                        unset($row[$key]);
                    }
                    return $row;
                });
    ...
    {
        if (!$this->importTranslations) return false;
    ...
    {
        $headerRow = [];
        if (empty($results)) return $headerRow;
        foreach ($results[0] as $key => $column) {
            if (!in_array($column, $this->headerColumns())) continue;
            if (in_array($column, $this->skipHeaderColumns())) continue;
            $headerRow[] = $column;
        }
        return $headerRow;
    }
    ...
    {
        $dataRows = [];
        if (empty($results)) return $dataRows;
        foreach ($results as $key => $row) {
            ...
            foreach ($row as $key2 => $value) {
                ...
                if (!in_array($columns[$key2 - $skippedColumns], $this->columns())) continue;
    ...
    {
        $relationShipRows = [];
        if (empty($results)) return $relationShipRows;
        foreach ($results as $key => $row) {
            $rowData = [];
            $skippedColumns = 0;
            foreach ($row as $key2 => $value) {
                ...
                if (!in_array($headerRow[$key2], $this->relationsColumns())) continue;
                $rowData[$headerRow[$key2]] = json_decode(base64_decode($value), true);
                if (is_null($rowData[$headerRow[$key2]])) continue;
    ...
    public function relationsColumns()
    {
        $columns = [];
        foreach ($this->relations() as $relation) {
            if (!array_key_exists('columnName', $relation)) continue;
